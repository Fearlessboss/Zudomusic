...
"""
╔══════════════════════════════════════════════════════════════════╗
║   ♫  Z U D O  X  M U S I C   —   v 14  U L T R A   F I X  ♫    ║
║   LIFETIME-STABLE CLONES • BLOCKQUOTE CARD • SETDP VIDEO        ║
║   SETTHUMB FIXED • HEARTBEAT WATCHDOG • PER-CLONE ISOLATION     ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ NEW IN v14 (over v13) ═══════════════════
  ✅ _save_state_sync FIX:
       → Directory ensured before write
       → tmp file path verified to exist before replace
       → Fallback to direct write if replace race occurs

  ✅ /vplay CLONE FIX:
       → command_arg now reads from text/caption properly
       → vplay filter also accepts caption commands
       → Better error surface (no silent swallow)
       → Stream builder verified for clone PyTgCalls instance

  ✅ CLONE STABILITY HARD-FIX:
       → Heartbeat writes BEFORE blocking calls (early proof of life)
       → Watchdog uses pid file mtime + boot grace better
       → spawn_clone_process: env passthrough fixed, no FD leak
       → Per-clone log rotation hint (size cap soft)
       → SIGTERM grace then SIGKILL — group cleanup guaranteed
       → Heartbeat written on bot startup IMMEDIATELY

  ✅ /setthmb PLAY CARD FIX:
       → _send_play_card now ALWAYS shows caption with custom thumb
       → Photo + video both render caption correctly
       → Edit path handles caption update without losing media
       → Track thumb fallback also carries caption

  ✅ BLOCKQUOTE PLAY CARD FINAL:
       → Each field in own <blockquote> tag
       → Title block with proper separator
       → No expandable flag (all visible at once)
       → Quote text appears in thumbnail caption properly
"""
...
# v14: Heartbeat config
HEARTBEAT_INTERVAL        = int(os.getenv("HEARTBEAT_INTERVAL", "20") or "20")
HEARTBEAT_STALE_AFTER     = int(os.getenv("HEARTBEAT_STALE_AFTER", "90") or "90")
WATCHDOG_INTERVAL         = int(os.getenv("WATCHDOG_INTERVAL", "25") or "25")
CLONE_BOOT_GRACE          = int(os.getenv("CLONE_BOOT_GRACE", "90") or "90")
...
# ═══════════════════════════════════════════════════════════════════
# SAFE ATOMIC WRITE (v14 — fixes _save_state FileNotFoundError)
# ═══════════════════════════════════════════════════════════════════

def safe_atomic_write(target: Path, content: str) -> bool:
    """
    Atomic write with full safety:
      1. Ensure parent directory exists
      2. Write tmp file
      3. Verify tmp exists before replace
      4. Fallback: direct write if replace fails
    ...
    try:
        ...
        try:
            ...
            # Parent dir vanished between mkdir and write — retry
            target.parent.mkdir(parents=True, exist_ok=True)
            tmp.write_text(content, encoding="utf-8")

        # Verify tmp actually exists before replace
        if not tmp.exists():
            # tmp got deleted somehow — direct write fallback
        ...
        try:
            ...
            # tmp disappeared between exists check and replace — direct write
            target.write_text(content, encoding="utf-8")
        return True
    except Exception:
        ...
        try:
            ...
            log.exception("safe_atomic_write totally failed for %s", target)
            return False
...
def save_shared_thumb(media_type: str, file_id: str) -> None:
    content = json.dumps({"type": media_type, "file_id": file_id}, indent=2)
    safe_atomic_write(SHARED_THUMB_FILE, content)
...
def write_heartbeat(bot_id: str) -> None:
    ...
    try:
        ...
        safe_atomic_write(hb, str(int(time.time())))
    except Exception:
        pass
...
def command_arg(message: Message) -> str:
    """v14: read from text OR caption, strip command robustly."""
    text = (message.text or message.caption or "") or ""
    text = text.strip()
    if not text:
        return ""
    # Split off the command (first token)
...
def save_config(cfg: BotConfig, path: Path) -> None:
    ...
    safe_atomic_write(path, json.dumps(asdict(cfg), indent=2, ensure_ascii=False))
...
def spawn_clone_process(cfg_file: Path, bot_id: str) -> Optional[int]:
    ...
    for attempt in range(3):
        try:
            ...
            try:
                env = os.environ.copy()
                env["ZUDO_CLONE_ID"] = bot_id
                env["ZUDO_BOT_ID"] = bot_id
...
def classify_error(
    ...
    *,
    ...
    return (
        ...
        f"<blockquote>{escape_html(raw[:200]) if raw else 'unknown error'}\n\n"
        "ᴋʀɪᴘʏᴀ ᴋᴜᴄʜ ꜱᴇᴄᴏɴᴅꜱ ʙᴀᴀᴅ /play ᴅᴜʙᴀʀᴀ ᴄʜᴀʟᴀᴏ. 🙏</blockquote>"
    )
...
class TelegramMusicBot:
    ...
    # ─── PERSISTENT STATE (v14 FIX: directory-safe atomic write) ───

    def _save_state_sync(self) -> None:
        """
        v14 FIX: Use safe_atomic_write which:
          - Ensures parent directory exists
          - Verifies tmp before replace
          - Falls back to direct write if race occurs
        """
        try:
            data = {}
            ...
            ok = safe_atomic_write(self.state_file, content)
            if not ok:
                log.warning("save_state could not write to %s", self.state_file)
        except Exception:
    ...
    def _save_known(self) -> None:
        safe_atomic_write(USERS_FILE, json.dumps(list(self.known_users)))
        safe_atomic_write(CHATS_FILE, json.dumps(list(self.known_chats)))
    ...
    def _save_settings(self) -> None:
        try:
            content = json.dumps(self.settings, indent=2, ensure_ascii=False)
            safe_atomic_write(self.settings_path, content)
        except Exception:
    ...
    # ═══════════════ BLOCKQUOTE PLAY-CARD (v14 FINAL) ═══════════════

    def _play_card_text(self, track: Track) -> str:
        """
        v14: Each field in its OWN <blockquote> for proper quoted boxes.
        Telegram renders each <blockquote> as a separate green-bar quoted box.
        Caption (this text) is sent ALONG WITH the thumbnail media.
        """
        ...
        req = track.requested_by  # already escaped via mention_user
    ...
    # ═══════════════════════════════════════════════════════════════
    # SEND PLAY CARD (v14 FIX: custom thumb + caption GUARANTEED)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _send_play_card(self, chat_id: int, track: Track,
        ...
        """
        v14 FIX: Play card sends caption WITH thumbnail every time.
        Priority:
          1. Custom global /setthmb (photo OR video) + caption
          2. YouTube track thumbnail + caption
          3. Plain text caption only
        """
        text = self._play_card_text(track)
        kb   = self._play_card_kb()
        thumb = load_shared_thumb()
        media_type = (thumb.get("type") or "").lower()
        file_id    = (thumb.get("file_id") or "").strip()
        has_custom = bool(file_id and media_type in ("photo", "video"))

        # ── EDIT PATH (only if media types match) ──
        if edit_msg is not None:
            try:
                ...
                want_text     = (not has_custom) and (not track.thumbnail)
        ...
        # ── FRESH SEND PATH ──
        # Priority 1: Custom global thumb set via /setthmb (PHOTO) + caption
        if has_custom and media_type == "photo":
            try:
                ...
                try:
                    return await self.bot.send_photo(
                        chat_id=chat_id, photo=file_id, caption=text, reply_markup=kb,
                    )
                except Exception as e:
                    log.warning("custom photo thumb retry failed: %s", e)
            except Exception as e:
                log.warning("custom photo thumb send failed: %s — falling back", e)

        # Priority 1b: Custom global thumb (VIDEO) + caption
        if has_custom and media_type == "video":
            try:
                ...
                try:
                    return await self.bot.send_video(
                        chat_id=chat_id, video=file_id, caption=text, reply_markup=kb,
                        supports_streaming=True,
                    )
                except Exception as e:
                    log.warning("custom video thumb retry failed: %s", e)
            except Exception as e:
                log.warning("custom video thumb send failed: %s — falling back", e)

        # Priority 2: YouTube track thumbnail + caption
        if track.thumbnail:
            try:
                return await self.bot.send_photo(
                    chat_id=chat_id,
                ...
                log.warning("yt thumbnail send failed: %s — falling back to text", e)
        ...
        try:
            ...
            try:
                return await self.bot.send_message(
                    chat_id=chat_id, text=text, reply_markup=kb, disable_web_page_preview=True,
                )
            except Exception:
                pass
        except Exception:
            log.exception("send_play_card final text send failed")
        return None
    ...
    # ═══════════════════════════════════════════════════════════════
    # STREAM BUILDER (v14 verified for both audio + video)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _pytgcalls_play(self, chat_id: int, url: str, is_video: bool) -> None:
        ...
        if not streams:
            raise RuntimeError(f"No stream builder available for is_video={is_video}")
        last_exc: Optional[Exception] = None
    ...
    # ─── CORE PLAY HANDLER (v14 FIX: /vplay clones) ───
    ...
    async def _handle_play(self, message: Message, query: str, want_video: bool = False) -> None:
        ...
        try:
            ...
            log.exception("extract_track failed (video=%s, query=%s)", want_video, query[:60])
        ...
        async with self.get_lock(message.chat.id):
            ...
            try:
                ...
                log.exception("play_track failed (video=%s)", want_video)
                state.current = None
                state.paused = False
                self._schedule_save()
                friendly = classify_error(
        ...
        # Send play card AFTER playing successfully
        try:
    ...
    async def _add_handlers(self) -> None:
        ...
        async def play(_, m: Message):
            try:
                arg = command_arg(m)
                await self._handle_play(m, arg, want_video=False)
            except Exception:
                log.exception("play handler failed")
                try:
                    await self._safe_send(m, "❌ /play ᴍᴇ ᴛᴇᴄʜɴɪᴄᴀʟ ɪꜱꜱᴜᴇ — ᴅᴜʙᴀʀᴀ ᴛʀʏ ᴋᴀʀᴏ.")
                except Exception: pass

        # ── /vplay /vp (VIDEO ONLY) — v14 FIX ──
        @self.bot.on_message(filters.command(["vplay", "vp"]) & filters.group)
        async def vplay(_, m: Message):
            try:
                arg = command_arg(m)
                log.info("vplay invoked: chat=%s arg=%r", m.chat.id, arg[:80])
                await self._handle_play(m, arg, want_video=True)
            except Exception as e:
                log.exception("vplay handler failed")
                try:
                    await self._safe_send(
                        m,
                        f"❌ <b>/vplay ᴇʀʀᴏʀ</b>\n\n"
                        f"<blockquote>{escape_html(str(e)[:200])}</blockquote>\n"
                        f"ᴅᴜʙᴀʀᴀ ᴛʀʏ ᴋᴀʀᴏ ʏᴀ /play ᴜꜱᴇ ᴋᴀʀᴏ."
                    )
                except Exception: pass
    ...
    async def start(self) -> None:
        ...
        # v14: write heartbeat IMMEDIATELY on start — prevents watchdog killing during boot
        write_heartbeat(self.config.bot_id)
        ...
        write_heartbeat(self.config.bot_id)  # heartbeat after asst start
        ...
        write_heartbeat(self.config.bot_id)  # heartbeat after bot start
        ...
        write_heartbeat(self.config.bot_id)  # heartbeat after pytgcalls
        log.info("🚀 ONLINE | %s | @%s | id=%s", self.bot_name, self.bot_username, self.config.bot_id)

        # Start heartbeat loop ASAP
...
async def supervisor() -> None:
    ...
    while True:
        ...
        # Reset delay on long-running successful sessions
