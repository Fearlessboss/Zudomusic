...
║   Premium UI • Thumbnail Sync • Quoted Play Card • Color Btns   ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ NEW IN v9 (over v8) ═══════════════════
  ✅ Premium play status card (quoted style like demo SS):
       → TITLE / DURATION / REQUESTED with ❝ ❞ accents
       → NO "Powered by" line
  ✅ Colored premium emoji buttons (Tunes / Home / Close)
  ✅ /setthmb command (main bot owner only):
       → set video OR photo as universal thumbnail
       → automatically syncs to ALL clone bots (shared file)
       → appears on every /play status (main + clones)
  ✅ Close button deletes the play status message
  ✅ All v8 fixes retained (skip, audio-only, flood-wait, etc.)
"""
...
# ✨ NEW: shared thumbnail file → main bot sets it, all clones read from here
...
# ═══════════════════════════════════════════════════════════════════
# ✨ NEW: SHARED THUMBNAIL HELPERS
# main bot writes → clones read (filesystem-shared)
# ═══════════════════════════════════════════════════════════════════
...
class TelegramMusicBot:
    ...
    def __init__(self, config: BotConfig, config_path: Optional[Path] = None, is_master: bool = False):
        ...
        self.pending_thumb:       Dict[int, float]          = {}  # ✨ NEW
    ...
    # ═══════════════════════════════════════════════════════
    # ✨ NEW: PREMIUM QUOTED PLAY-CARD (like the demo SS)
    # ═══════════════════════════════════════════════════════

    def _play_card_text(self, track: Track) -> str:
        """Premium quoted card matching the demo screenshot style.
        NO 'Powered by' line as per instructions."""
        mode_emoji = "📹" if track.is_video else "🎵"
        return (
            f"<b>🎶 {escape_html(self.display_name)}</b>\n"
            f"<i>{sep_thin()}</i>\n\n"
            f"<blockquote>"
            f"🍎  <b>ᴛɪᴛʟᴇ</b>     :  {escape_html(track.title)}    ❞\n"
            f"⚙️  <b>ᴅᴜʀᴀᴛɪᴏɴ</b>  :  {escape_html(track.pretty_duration)}\n"
            f"💫  <b>ʀᴇǫᴜᴇꜱᴛᴇᴅ</b> :  {track.requested_by}\n"
            f"{mode_emoji}  <b>ᴍᴏᴅᴇ</b>      :  {'ᴠɪᴅᴇᴏ' if track.is_video else 'ᴀᴜᴅɪᴏ'}"
            f"</blockquote>"
        )
    ...
    def _np_text(self, state: ChatState) -> str:
        ...
        t = state.current
        # Use the premium quoted card style + extra status info
    ...
    # ═══════════════════════════════════════════════════════
    # ✨ NEW: PREMIUM PLAY-CARD KEYBOARD (Tunes / Home / Close)
    # like the demo screenshot
    # ═══════════════════════════════════════════════════════
    ...
    def _play_card_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            ...
            [
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data="ctl_close_card"),
            ],
        ])
    ...
    def _queue_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🎵 ɴᴏᴡ ᴘʟᴀʏɪɴɢ", callback_data="ctl_np"),
             InlineKeyboardButton("🏠 ʜᴏᴍᴇ",        callback_data="nav_home")],
            [InlineKeyboardButton("❌ ᴄʟᴏꜱᴇ", callback_data="ctl_close_card")],
        ])

    # ═══════════════════════════════════════════════════════════════
    # ✨ NEW: SEND PREMIUM PLAY CARD (with shared thumbnail)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _send_play_card(self, chat_id: int, track: Track,
        ...
        """
        Send (or edit) the premium quoted play card.
        If a shared thumbnail (photo/video) is set, attach it as media.
        """
        ...
        # EDIT mode (used when transitioning from "🔎 searching..." text msg)
        if edit_msg is not None:
            try:
                # If the edit_msg already has media, just edit caption
                ...
                # If a thumbnail is configured → delete the text msg and send a new media msg
                ...
                log.exception("play_card edit failed — falling back to fresh send")
        ...
        try:
            ...
            # No shared thumb → fall back to track's own thumbnail URL (if any)
            if track.thumbnail:
                try:
                    return await self.bot.send_photo(
    ...
    async def _ensure_assistant_in_chat(self, chat_id: int) -> Tuple[bool, Optional[str]]:
        ...
        # Try cheap member check via bot first
        ...
        # Try warming peer directly (sometimes already a member)
        if await self._warm_peer(chat_id):
            self._assistant_joined[chat_id] = now
            return True, None

        # Need to join via invite link
        link = ""
        try:
            chat = await self.bot.get_chat(chat_id)
            uname = getattr(chat, "username", None)
            if uname:
                ...
                try:
                    ...
                    return False, (
                        f"⚠️ ʙᴏᴛ ᴀᴅᴍɪɴ ɴᴀʜɪ ʜᴀɪ ʏᴀ ɪɴᴠɪᴛᴇ ᴜꜱᴇʀꜱ ᴘᴇʀᴍ ᴅᴏ.\n"
                        f"{escape_html(str(e))}"
                    )
        except Exception as e:
            return False, (
                f"⚠️ ᴄʜᴀᴛ ɪɴꜰᴏ ɴᴀʜɪ ᴍɪʟɪ.\n"
                f"{escape_html(str(e))}"
            )
    ...
    # ═══════════════════════════════════════════════════════════════
    # PLAY NEXT (with clean-leave-rejoin for skip)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _handle_play(self, message: Message, query: str, want_video: bool = False) -> None:
        ...
        # Send the premium quoted play-card (with thumbnail if set)
        try:
            # Delete the "connecting..." text msg cleanly, then send card
    ...
    async def _add_handlers(self) -> None:
        ...
        async def cb(_, q):
            try:
                ...
                # ✨ Close-card → just delete the play card message
                if d == "ctl_close_card":
                    try: await q.message.delete()
                    except Exception: pass
                    return await q.answer("ᴄʟᴏꜱᴇᴅ")
        ...
        # ═══════════════════ ✨ NEW: GLOBAL PLAY THUMBNAIL ═══════════════════
        # Only MASTER bot owner can set the global thumbnail.
        # Saved to shared file → all clones read the same thumbnail.
        # ════════════════════════════════════════════════════════════════════
        ...
        # ── Photo/Video receiver (for /setdp AND /setthmb) ──
            ...
            try:
                ...
                if self.is_master and m.from_user and m.from_user.id in self.pending_thumb:
                    ...
                    if m.video:
                        ...
                        fid = m.animation.file_id; media_type = "video"  # animations work like videos
                    elif m.photo:
                        po = m.photo
                ...
                if m.from_user and m.from_user.id in self.pending_start_photo:
                    ...
                    self._save_settings()
                    self.pending_start_photo.pop(m.from_user.id, None)
                    return await self._safe_send(m, f"✅ ꜱᴀᴠᴇᴅ! /start ᴘᴇ ᴅɪᴋʜᴇɢɪ.")
            except Exception:
    ...
    async def start(self) -> None:
        ...
        # Enable HTML parse mode by default for premium formatting
        try:
