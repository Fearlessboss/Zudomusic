...
"""
╔══════════════════════════════════════════════════════════════════╗
║      ♫  Z U D O  X  M U S I C   —   v 9   P R E M I U M  ♫      ║
║   Premium Now-Playing Panel • Colored Buttons • Quote Style     ║
║   Master /setthmb → Auto-Sync to ALL Clones                     ║
║   Clone-Skip Fix • Audio-Only Play • Flood-Wait Killer          ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ NEW IN v9 ═══════════════════
  ✨ Premium "Now Playing" card (matches your screenshot style):
       → Quote-style title block
       → Premium emojis (🍎 🌸 🎀 🍷 🍇 etc.)
       → TITLE / DURATION / REQUESTED in blockquote
       → Colorful inline buttons (TUNES / HOME / CLOSE)
       → 5:17 ─────────────── 5:20  progress bar
  ✨ /setthmb (master + clone, owner only):
       → Send video / photo → set as global thumbnail
       → Saved in MASTER bot, auto-synced to all clones via shared file
       → Used in every /play "Now Playing" message
  ✨ /delthmb → clears thumbnail
  ✨ Close (🗑) button deletes the play message
  ✨ TUNES (📀) button → opens player controls
  ✨ HOME (⚡) button → opens main start panel
  ✨ All previous v8 fixes retained (skip / flood-wait / audio-only)
"""
...
# ✨ Shared global thumbnail file — master saves, ALL clones read from here
GLOBAL_THUMB_FILE = ROOT_RUNTIME_DIR / "global_thumb.json"

for d in (CLONES_DIR, LOGS_DIR, PIDS_DIR, STATES_DIR, CONTROL_DIR):
    d.mkdir(parents=True, exist_ok=True)

# ═══════════════════════════════════════════════════════════════════
# PREMIUM EMOJIS (used in UI)
# ═══════════════════════════════════════════════════════════════════
# Format: "{ascii_emoji}" with optional custom_emoji_id wrap (Telegram premium)
# We use entities-style HTML <tg-emoji emoji-id="ID">FALLBACK</tg-emoji>
# so non-premium users still see the fallback emoji, premium users see custom.

PE_APPLE     = '<tg-emoji emoji-id="5870870747363791008">🍎</tg-emoji>'
PE_GEAR      = '<tg-emoji emoji-id="5870747177772713186">⚙️</tg-emoji>'
PE_SPARKLE   = '<tg-emoji emoji-id="5870889550985673194">✨</tg-emoji>'
PE_WINE      = '<tg-emoji emoji-id="5870786008555857758">🍷</tg-emoji>'
PE_HEART     = '<tg-emoji emoji-id="5870654844708263791">💜</tg-emoji>'
PE_HEADPHONE = '<tg-emoji emoji-id="5870691383478870541">🎧</tg-emoji>'
PE_FIRE      = '<tg-emoji emoji-id="5870740619007375431">🔥</tg-emoji>'
PE_STAR      = '<tg-emoji emoji-id="5870747100340961585">⭐</tg-emoji>'
PE_GIRL      = '<tg-emoji emoji-id="5870881729184729951">👧</tg-emoji>'
PE_DISC      = '<tg-emoji emoji-id="5870746626591985875">📀</tg-emoji>'
PE_HOME      = '<tg-emoji emoji-id="5870884326606987326">🏠</tg-emoji>'
PE_CLOSE     = '<tg-emoji emoji-id="5870661839856792000">🗑</tg-emoji>'
PE_BOLT      = '<tg-emoji emoji-id="5870791052010628507">⚡</tg-emoji>'
PE_PLAY      = '<tg-emoji emoji-id="5870868637483987144">▶️</tg-emoji>'
PE_PAUSE     = '<tg-emoji emoji-id="5870867050636635867">⏸</tg-emoji>'
PE_SKIP      = '<tg-emoji emoji-id="5870871140728115060">⏭</tg-emoji>'
PE_STOP      = '<tg-emoji emoji-id="5870870016358563125">⏹</tg-emoji>'
PE_QUEUE     = '<tg-emoji emoji-id="5870870064603051345">📜</tg-emoji>'
PE_MUTE      = '<tg-emoji emoji-id="5870870167942030008">🔇</tg-emoji>'
...
# ═══════════════════════════════════════════════════════════════════
# GLOBAL THUMBNAIL — shared between master & clones
# ═══════════════════════════════════════════════════════════════════

def read_global_thumb() -> Dict[str, str]:
    """Returns {'type': 'photo'|'video'|'', 'file_id': '...'}."""
    try:
        if GLOBAL_THUMB_FILE.exists():
            data = json.loads(GLOBAL_THUMB_FILE.read_text(encoding="utf-8"))
...
def write_global_thumb(media_type: str, file_id: str) -> None:
    try:
        tmp = GLOBAL_THUMB_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps({"type": media_type, "file_id": file_id},
                                  indent=2, ensure_ascii=False), encoding="utf-8")
        tmp.replace(GLOBAL_THUMB_FILE)
    except Exception:
        log.exception("write_global_thumb failed")

def clear_global_thumb() -> None:
    try:
        if GLOBAL_THUMB_FILE.exists():
            GLOBAL_THUMB_FILE.unlink()
    except Exception:
        pass
...
def progress_bar(duration: int) -> str:
    """Fake progress bar: 00:00 ──────────⬜ MM:SS  (since real position not exposed)"""
    if not duration or duration <= 0:
        return "▶️ ʟɪᴠᴇ ────────────────"
    m, s = divmod(int(duration), 60)
    end = f"{m:02d}:{s:02d}"
    # Show ~95% filled to mimic "now playing" feel
    bar = "━━━━━━━━━━━━━━━━━━━━ ⬜"
    return f"00:00 {bar} {end}"
...
def _make_ydl_opts(want_video: bool, client_index: int = 0) -> dict:
    ...
    if want_video:
        fmt = "bestvideo[ext=mp4][height<=720]+bestaudio/best[ext=mp4]/best"
    else:
        fmt = "bestaudio[ext=m4a]/bestaudio/best[vcodec=none]/best"
    return {
        "format": fmt,
        "quiet": True,
        "no_warnings": True,
...
class TelegramMusicBot:
    ...
    def __init__(self, config: BotConfig, config_path: Optional[Path] = None, is_master: bool = False):
        ...
        self.pending_thumb:       Dict[int, float]          = {}  # ✨ NEW: for /setthmb
    ...
    def _start_text(self, user_name: str = "") -> str:
        ...
        return (
            f"  {PE_SPARKLE} ʜᴇʟʟᴏ {greet_name}  {PE_SPARKLE}\n\n"
            f"  ɪ ᴀᴍ {n} {PE_HEADPHONE}\n"
            f"  ʏᴏᴜʀ ᴄᴜᴛᴇ ʟɪʟ ᴍᴜꜱɪᴄ ʙᴜᴅᴅʏ {PE_HEART}\n\n"
            f"  {sep_thin()}\n\n"
            f"  {PE_BOLT}  ꜱᴜᴘᴇʀ ꜰᴀꜱᴛ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ᴘʟᴀʏᴇʀ\n"
            f"  {PE_DISC}  ᴀᴜᴅɪᴏ + ᴠɪᴅᴇᴏ + ǫᴜᴇᴜᴇ ꜱᴜᴘᴘᴏʀᴛ\n"
            f"  {PE_STAR}  24×7 ᴏɴʟɪɴᴇ • ᴢᴇʀᴏ ʟᴀɢ\n\n"
    ...
    def _about_text(self) -> str:
        n = escape_html(self.display_name)
        return (
            f"  {PE_SPARKLE} ᴀʙᴏᴜᴛ {n} {PE_SPARKLE}\n\n"
            f"  <blockquote>❝ ᴍᴜsɪᴄ ɪs ᴛʜᴇ sʜᴏʀᴛʜᴀɴᴅ ᴏꜰ ᴇᴍᴏᴛɪᴏɴ. ❞</blockquote>\n\n"
            f"  {PE_FIRE}  ꜰᴇᴀᴛᴜʀᴇꜱ\n"
            ...
            f"  {PE_BOLT} ɢʀᴏᴜᴘ ꜱᴇᴛᴜᴘ:\n"
            f"  ❶ ʙᴏᴛ ᴀᴅᴅ ᴋᴀʀᴏ\n"
            f"  ❷ ᴀᴅᴍɪɴ ʙᴀɴᴀᴏ\n"
            f"  ❸ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ꜱᴛᴀʀᴛ ᴋᴀʀᴏ\n"
            f"  ❹ /play ꜱᴏɴɢ ɴᴀᴍᴇ {PE_DISC}"
        )

    def _help_home_text(self) -> str:
        n = escape_html(self.display_name)
        return (
            f"  {PE_QUEUE} {n} — ʜᴇʟᴘ ᴄᴇɴᴛᴇʀ\n\n"
            f"  <blockquote>❝ ᴛʜᴇ ʙᴇsᴛ ᴍᴜsɪᴄ ᴍᴀᴋᴇs ʏᴏᴜ ᴅᴀɴᴄᴇ. ❞</blockquote>\n\n"
    ...
    def _help_basic_text(self) -> str:
        return (
            f"  ⊚ ʙᴀsɪᴄ &amp; ᴛᴏᴏʟs\n\n"
            f"  🚀 /start : sᴛᴀʀᴛ ʙᴏᴛ.\n"
            f"  ❓ /help : ʜᴇʟᴩ ᴍᴇɴᴜ.\n"
            f"  📡 /ping : sʜᴏᴡ ᴩɪɴɢ &amp; sʏsᴛᴇᴍ sᴛᴀᴛs.\n"
            f"  📊 /stats : sʜᴏᴡ ʙᴏᴛ sᴛᴀᴛs.\n"
            f"  🎧 /song [ɴᴀᴍᴇ/ᴜʀʟ] : ᴅᴏᴡɴʟᴏᴀᴅ sᴏɴɢ.\n"
            f"  🔄 /reload : ʀᴇғʀᴇsʜ ᴀᴅᴍɪɴ ᴄᴀᴄʜᴇ.\n"
            f"  ♻️ /reboot ᴏʀ /restart : ʀᴇʙᴏᴏᴛ ʙᴏᴛ ɪɴ ᴄʜᴀᴛ.\n"
            f"  👑 /addsudo ᴏʀ /delsudo : ᴀᴅᴅ ᴏʀ ʀᴇᴍᴏᴠᴇ sᴜᴅᴏ ᴜsᴇʀs.\n"
            f"  📋 /sudolist : sʜᴏᴡ ᴀʟʟ sᴜᴅᴏ ᴜsᴇʀs."
        )

    def _help_music_text(self) -> str:
        return (
            f"  {PE_DISC} ᴍᴜꜱɪᴄ ᴄᴏᴍᴍᴀɴᴅꜱ\n\n"
            f"  ▸  /play  sᴏɴɢ / ᴜʀʟ  →  {PE_DISC} ᴀᴜᴅɪᴏ ᴏɴʟʏ\n"
            f"  ▸  /vplay sᴏɴɢ / ᴜʀʟ  →  📹 ᴠɪᴅᴇᴏ ᴏɴʟʏ\n"
            f"  ▸  /p     sᴏɴɢ        →  /play ᴀʟɪᴀs\n\n"
            f"  {PE_PAUSE}  /pause    →  ᴘᴀᴜꜱᴇ\n"
            f"  {PE_PLAY}  /resume   →  ʀᴇꜱᴜᴍᴇ\n"
            f"  {PE_SKIP}  /skip     →  ɴᴇxᴛ ᴛʀᴀᴄᴋ\n"
            f"  {PE_STOP}  /stop     →  ꜱᴛᴏᴘ + ᴄʟᴇᴀʀ\n"
            f"  {PE_QUEUE}  /queue    →  ᴠɪᴇᴡ ǫᴜᴇᴜᴇ\n"
            f"  {PE_DISC}  /np       →  ɴᴏᴡ ᴘʟᴀʏɪɴɢ\n"
    ...
    def _shell_help_text(self) -> str:
        return (
            ...
            f"  ▸  /setthmb    →  🌸 ᴘʟᴀʏ ᴛʜᴜᴍʙɴᴀɪʟ ꜱᴇᴛ (ɢʟᴏʙᴀʟ)\n"
            f"  ▸  /delthmb    →  ᴛʜᴜᴍʙ ʜᴀᴛᴀᴏ\n\n"
    ...
    def _np_card_text(self, state: ChatState) -> str:
        """Premium Now-Playing card — quote style, premium emojis, progress bar."""
        if not state.current:
            return (
                f"  {PE_DISC} ɴᴏᴡ ᴘʟᴀʏɪɴɢ\n\n"
                f"  ❌ ᴀʙʜɪ ᴋᴜᴄʜ ᴘʟᴀʏ ɴᴀʜɪ ʜᴏ ʀᴀʜᴀ.\n\n"
                f"  💡 /play sᴏɴɢ ɴᴀᴍᴇ"
            )
        t = state.current
        title = escape_html(t.title)
        dur   = escape_html(t.pretty_duration)
        req   = t.requested_by
        bar   = progress_bar(t.duration)

        # Quote-style block (matches your screenshot demo)
        body = (
            f"<blockquote>"
            f"{PE_APPLE} <b>ᴛɪᴛʟᴇ</b>: {title}\n"
            f"{PE_GEAR} <b>ᴅᴜʀᴀᴛɪᴏɴ</b>: {dur}\n"
            f"{PE_HEART} <b>ʀᴇǫᴜᴇsᴛᴇᴅ</b>: {req}"
            f"</blockquote>\n\n"
            f"{PE_GIRL} <code>{bar}</code>"
        )
        return body

    def _queue_text(self, state: ChatState) -> str:
        if not state.current and not state.queue:
            return f"  {PE_QUEUE} ǫᴜᴇᴜᴇ\n\n  📭 ǫᴜᴇᴜᴇ ᴇᴍᴘᴛʏ ʜᴀɪ."
        lines = [f"  {PE_QUEUE} ǫᴜᴇᴜᴇ", ""]
        if state.current:
            lines.append(f"  {PE_DISC} ᴘʟᴀʏɪɴɢ:")
            lines.append(f"      {escape_html(state.current.title)}")
            lines.append(f"      ⏱ {escape_html(state.current.pretty_duration)}")
            lines.append("")
        if state.queue:
            lines.append(f"  {PE_SKIP} ᴜᴘ ɴᴇxᴛ:")
        ...
        lines.append("")
        lines.append(f"  🔁 {human_bool(state.loop)}   {PE_PAUSE} {human_bool(state.paused)}")
        return "\n".join(lines)
    ...
    def _start_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            [InlineKeyboardButton(f"➕ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ➕", url=self.add_to_group_url)],
    ...
    def _help_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("⊚ ʙᴀꜱɪᴄ", callback_data="help_basic"),
    ...
    # ✨ PREMIUM NOW-PLAYING KEYBOARD (matches screenshot)
    def _np_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            # Row 1: skip-back / pause / skip-forward / close
            [
                InlineKeyboardButton("⏮", callback_data="ctl_skip_back"),
                InlineKeyboardButton("⏸", callback_data="ctl_pause"),
                InlineKeyboardButton("⏭", callback_data="ctl_skip"),
                InlineKeyboardButton("🗑", callback_data="nav_close"),
            ],
            # Row 2: TUNES (player) | HOME
            [
                InlineKeyboardButton("📀 ᴛᴜɴᴇs", callback_data="ctl_tunes"),
                InlineKeyboardButton("🛡 ʜᴏᴍᴇ", callback_data="nav_home"),
            ],
    ...
    def _tunes_kb(self) -> InlineKeyboardMarkup:
        """Extended controls (revealed when TUNES tapped)."""
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⬅ ʙᴀᴄᴋ", callback_data="ctl_np"),
             InlineKeyboardButton("🗑 ᴄʟᴏꜱᴇ", callback_data="nav_close")],
        ])
    ...
    # ═══════════════════════════════════════════════════════════════
    # ✨ SMART PEER / JOIN ENGINE (Flood-Wait killer)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _ensure_assistant_in_chat(self, chat_id: int) -> Tuple[bool, Optional[str]]:
        ...
        # 2) flood cooldown check
        flood_ts = self._join_flood_until.get(chat_id, 0)
        if flood_ts and now < flood_ts:
            if await self._warm_peer(chat_id):
                self._assistant_joined[chat_id] = now
                return True, None
            return False, (
                f"⚠️ ᴊᴏɪɴ ꜰʟᴏᴏᴅ-ᴡᴀɪᴛ ᴄᴏᴏʟᴅᴏᴡɴ ʙᴀᴄʜᴀ ʜᴀɪ.\n"
                f"ᴀꜱꜱɪꜱᴛᴀɴᴛ @{escape_html(self.assistant_username)} ᴋᴏ ᴍᴀɴᴜᴀʟʟʏ ᴀᴅᴅ ᴋᴀʀ ᴅᴏ."
            )

        # 3) Check membership via bot first
        member = await self._is_assistant_member(chat_id)
        if member is True:
            await self._warm_peer(chat_id)
            self._assistant_joined[chat_id] = now
            return True, None

        # 4) Try peer warm without join
        if await self._warm_peer(chat_id):
            self._assistant_joined[chat_id] = now
            return True, None

        # 5) Final fallback: try invite link join
        try:
            link = await self.bot.export_chat_invite_link(chat_id)
        except Exception as e:
            return False, (
                f"⚠️ ɪɴᴠɪᴛᴇ ʟɪɴᴋ ɴᴀʜɪ ᴍɪʟᴀ — ʙᴏᴛ ᴋᴏ ᴀᴅᴍɪɴ ʙᴀɴᴀᴏ + ɪɴᴠɪᴛᴇ ᴜꜱᴇʀꜱ ᴘᴇʀᴍ ᴅᴏ.\n"
                f"{escape_html(str(e))}"
            )
        ...
        try:
            ...
            if await self._warm_peer(chat_id):
                ...
                return True, None
            return False, (
                f"⚠️ ᴊᴏɪɴ ꜰʟᴏᴏᴅ-ᴡᴀɪᴛ: {wait_secs}ꜱ\n"
                f"ᴀꜱꜱɪꜱᴛᴀɴᴛ @{escape_html(self.assistant_username)} ᴋᴏ "
            ...
            if "FLOOD" in err:
                ...
                if await self._warm_peer(chat_id):
                    self._assistant_joined[chat_id] = now
                    return True, None
            return False, f"⚠️ ᴊᴏɪɴ ɴᴀʜɪ ʜᴜᴀ: {escape_html(str(e))}"
    ...
    # ═══════════════════════════════════════════════════════════════
    # STREAM BUILDER (audio strict / video strict)
    # ═══════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════
    # _play_next with CLEAN-LEAVE-THEN-REJOIN for skip
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _play_next_locked(self, chat_id: int, announce: bool = False,
        ...
        if announce:
            await self._send_np_card(chat_id, state)
    ...
    # ═══════════════════════════════════════════════════════════════
    # ✨ PREMIUM NP CARD SENDER (with global thumbnail)
    # ═══════════════════════════════════════════════════════════════

    async def _send_np_card(self, chat_id: int, state: ChatState) -> Optional[Message]:
        """Sends the premium Now-Playing card, using global thumbnail if set."""
        text = self._np_card_text(state)
        kb   = self._np_kb()
        thumb = read_global_thumb()
        tid   = thumb.get("file_id", "")
        ttype = thumb.get("type", "")

        if tid and ttype == "video":
            try:
                return await self.bot.send_video(
                    chat_id, video=tid, caption=text, reply_markup=kb,
                )
            except Exception:
                pass
        if tid and ttype == "photo":
            try:
                return await self.bot.send_photo(
                    chat_id, photo=tid, caption=text, reply_markup=kb,
                )
            except Exception:
                pass
        ...
        try:
            ...
            log.exception("send_np_card failed")
        return None
    ...
    async def _handle_play(self, message: Message, query: str, want_video: bool = False) -> None:
        ...
        # Delete the "connecting..." text msg and send the premium card fresh
        try:
            await msg.delete()
        except Exception:
            pass

        await self._send_np_card(message.chat.id, final_state)
    ...
    async def _add_handlers(self) -> None:
        ...
        async def cb(_, q):
            try:
                ...
                if d == "nav_help_home":
                    ...
                    return await q.answer()
                if d == "help_basic":
                    await self._safe_edit_panel(q.message, self._help_basic_text(), self._subpage_kb())
                    return await q.answer()
                if d == "help_music":
                ...
                if d.startswith("ctl_"):
                    ...
                    if d == "ctl_tunes":
                        await self._safe_edit_panel(q.message, self._np_card_text(state), self._tunes_kb())
                        return await q.answer()

                    if d == "ctl_np":
                        await self._safe_edit_panel(q.message, self._np_card_text(state), self._np_kb())
                        return await q.answer()
                    ...
                    if d == "ctl_skip" or d == "ctl_skip_back":
                    ...
                    if d == "ctl_shuffle":
                        if len(state.queue) < 2:
                            return await q.answer("ǫᴜᴇᴜᴇ ᴍᴇ 2 ꜱᴇ ᴋᴀᴍ ᴛʀᴀᴄᴋꜱ.", show_alert=True)
                    ...
                    if d == "ctl_clearqueue":
                        ...
                        await self._safe_edit_panel(q.message, self._queue_text(state), self._queue_kb())
                        return await q.answer(f"🧹 {c} ᴄʟᴇᴀʀᴇᴅ")
        ...
        async def refresh(_, m: Message):
            try:
                ...
                await self._send_np_card(m.chat.id, state)
            except Exception: pass
        ...
        async def shuffle(_, m: Message):
            try:
                ...
                state = self.get_state(m.chat.id)
                if len(state.queue) < 2:
                    return await self._safe_send(m, "ǫᴜᴇᴜᴇ ᴍᴇ 2 ꜱᴇ ᴋᴀᴍ ᴛʀᴀᴄᴋꜱ.")
                random.shuffle(state.queue); self._schedule_save()
        ...
        async def volume(_, m: Message):
            try:
                ...
                if not arg.isdigit():
                    state = self.get_state(m.chat.id)
                    return await self._safe_send(m, f"🔊 ᴄᴜʀʀᴇɴᴛ ᴠᴏʟᴜᴍᴇ: {state.volume}%\n\nᴜꜱᴀɢᴇ: /volume 1-200")
                v = max(1, min(200, int(arg)))
                try:
                    await self._change_volume(m.chat.id, v)
                    self.get_state(m.chat.id).volume = v
                    self._schedule_save()
                    await self._safe_send(m, f"🔊 ᴠᴏʟᴜᴍᴇ ꜱᴇᴛ: {v}%")
                except Exception as e:
        ...
        async def ubjoin(_, m: Message):
            try:
                ...
                if ok:
                    await self._safe_send(m, f"✅ ᴀꜱꜱɪꜱᴛᴀɴᴛ ᴊᴏɪɴᴇᴅ!\n\n@{escape_html(self.assistant_username)}")
                else:
        ...
        async def setdp(_, m: Message):
            try:
                ...
                await self._safe_send(m, "🖼 ᴘʜᴏᴛᴏ ʙʜᴇᴊᴏ.\n/cancel ꜱᴇ ʙᴀɴᴅ.")
            except Exception: pass
        ...
        # ════════════ ✨ /setthmb — GLOBAL PLAY THUMBNAIL ✨ ════════════
        @self.bot.on_message(filters.command(["setthmb"]) & filters.private)
        async def setthmb(_, m: Message):
            try:
                ...
                await self._safe_send(m,
                    f"  🌸 ɢʟᴏʙᴀʟ ᴛʜᴜᴍʙɴᴀɪʟ ꜱᴇᴛᴜᴘ\n\n"
                    f"  ᴀʙ ᴍᴜᴊʜᴇ ᴇᴋ <b>ᴠɪᴅᴇᴏ</b> ʏᴀ <b>ᴘʜᴏᴛᴏ</b> ʙʜᴇᴊᴏ.\n"
                    f"  ʏᴇ ᴀᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ <b>ꜱᴀʀᴇ ᴄʟᴏɴᴇ ʙᴏᴛꜱ</b> ᴍᴇ ʙʜɪ ꜱᴇᴛ ʜᴏ ᴊᴀʏᴇɢᴀ.\n\n"
                    f"  /delthmb → ʜᴀᴛᴀᴏ\n"
                    f"  /cancel → ʙᴀɴᴅ"
                )
            except Exception: pass
        ...
        # ── /delthmb ──
        @self.bot.on_message(filters.command(["delthmb"]) & filters.private)
        async def delthmb(_, m: Message):
            try:
                ...
                clear_global_thumb()
                self.pending_thumb.pop(m.from_user.id, None)
                await self._safe_send(m, "✅ ɢʟᴏʙᴀʟ ᴛʜᴜᴍʙɴᴀɪʟ ʜᴀᴛᴀʏᴀ ɢᴀʏᴀ.\nᴀʙ ᴀʟʟ ʙᴏᴛꜱ ᴘᴜʀᴀɴᴀ ᴛᴇxᴛ-ᴏɴʟʏ ᴄᴀʀᴅ ᴜꜱᴇ ᴋᴀʀᴇɴɢᴇ.")
            except Exception: pass

        # ── Media handler for /setdp and /setthmb ──
        @self.bot.on_message(filters.private & (filters.photo | filters.video | filters.document))
        async def media_handler(_, m: Message):
            try:
                if not self.is_config_owner_user(m): return
                uid = m.from_user.id if m.from_user else 0

                # ✨ /setthmb pending — video or photo
                if uid in self.pending_thumb:
                    fid = ""
                    ftype = ""
                    if m.video:
                        fid = m.video.file_id
                        ...
                        if mt.startswith("video/"):
                            fid = m.document.file_id; ftype = "video"
                        elif mt.startswith("image/"):
                            fid = m.document.file_id; ftype = "photo"

                    if not fid or not ftype:
                        return await self._safe_send(m, "❌ ꜱɪʀꜰ ᴠɪᴅᴇᴏ ʏᴀ ᴘʜᴏᴛᴏ ʙʜᴇᴊᴏ.")

                    write_global_thumb(ftype, fid)
                    self.pending_thumb.pop(uid, None)
                    return await self._safe_send(m,
                        f"  ✅ ɢʟᴏʙᴀʟ ᴛʜᴜᴍʙɴᴀɪʟ ꜱᴀᴠᴇᴅ!\n\n"
                        f"  📦 ᴛʏᴘᴇ: <b>{ftype}</b>\n"
                        f"  🌐 ᴀʙ ʏᴇ <b>ᴀʟʟ ʙᴏᴛꜱ</b> (ᴍᴀꜱᴛᴇʀ + ᴄʟᴏɴᴇꜱ) ᴋᴇ /play ᴄᴀʀᴅ ᴍᴇ ᴅɪᴋʜᴇɢᴀ."
                    )

                # /setdp pending — photo only
                if uid in self.pending_start_photo:
                    fid = ""
                    if m.photo:
                        po = m.photo
                        fid = po.file_id if hasattr(po, "file_id") else (po[-1].file_id if isinstance(po, (list, tuple)) and po else "")
                    ...
                    self._save_settings()
                    self.pending_start_photo.pop(uid, None)
                    return await self._safe_send(m, "✅ ꜱᴀᴠᴇᴅ! /start ᴘᴇ ᴅɪᴋʜᴇɢɪ.")
            except Exception:
                log.exception("media_handler failed")
        ...
        if self.is_master:
            ...
            async def dclone(_, m: Message):
                try:
                    ...
                    if not killed and not cfg_removed:
                        return await self._safe_send(m, f"⚠️ Bot {bot_id} ɴᴀʜɪ ᴍɪʟᴀ.")
                    await self._safe_send(m,
                        f"  ✅ ʙᴏᴛ ꜱᴛᴏᴘᴘᴇᴅ\n\n"
                        f"  🤖  {bot_id}\n"
            ...
            async def cancel(_, m: Message):
                try:
                    if not self.is_config_owner_user(m): return
                    had = (m.from_user.id in self.clone_flow) or (m.from_user.id in self.pending_start_photo) or (m.from_user.id in self.pending_thumb)
                ...
                try:
                    ...
                    for f in files[:50]:
                        try:
                            ...
                            lines.append(f"  {'🟢' if live else '🔴'} {escape_html(cfg.bot_id)}")
            ...
            async def bc(_, m: Message):
                try:
                    ...
                    await self._safe_edit(status, f"  ✅ ʙʀᴏᴀᴅᴄᴀꜱᴛ ᴅᴏɴᴇ\n\n  ✓ ꜱᴇɴᴛ: {sent}\n  ✗ ꜰᴀɪʟᴇᴅ: {failed}")
                except Exception as e:
                    await self._safe_send(m, f"❌ {escape_html(str(e))}")
                ...
                try:
                    ...
                    if step == "session":
                        if text.lower() == "/default":
                            ...
                            ss = text
                        if len(ss) < 100:
                            return await self._safe_send(m, "❌ ɪɴᴠᴀʟɪᴅ ꜱᴇꜱꜱɪᴏɴ.")
