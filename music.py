...
"""
╔══════════════════════════════════════════════════════════════════╗
║   ♫  Z U D O  X  M U S I C   —   v 12  U L T I M A T E  ♫       ║
║   UNLIMITED CLONES • ZERO CRASH • HEARTBEAT WATCHDOG            ║
║   Blockquote Play Card • Real-Ban Detection • FD-Safe Spawn     ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ NEW IN v12 (over v11) ═══════════════════
  ✅ UNLIMITED CLONES — 5/10/50 koi bhi number, koi crash nahi:
       → Heartbeat-based watchdog (alive + responsive both check)
       → Proper FD detach via close_fds + stdin=DEVNULL
       → Process group isolation (os.setsid)
       → Per-clone isolated workdir (no file lock contention)
       → PyTgCalls auto-recovery on hang
       → Memory-bounded caches (no leak over time)
       → Stale heartbeat = automatic force-restart

  ✅ BLOCKQUOTE PLAY CARD — exactly like your screenshot:
       → Each field wrapped in <blockquote> (Telegram native)
       → Caption renders with proper quoted-text style on thumbnail

  ✅ All v11 features retained (smart-join, ban detection, etc.)
"""
...
# v12: Heartbeat config
HEARTBEAT_INTERVAL        = int(os.getenv("HEARTBEAT_INTERVAL", "30") or "30")
HEARTBEAT_STALE_AFTER     = int(os.getenv("HEARTBEAT_STALE_AFTER", "120") or "120")
WATCHDOG_INTERVAL         = int(os.getenv("WATCHDOG_INTERVAL", "20") or "20")
CLONE_BOOT_GRACE          = int(os.getenv("CLONE_BOOT_GRACE", "60") or "60")
...
HEARTBEAT_DIR = ROOT_RUNTIME_DIR / "heartbeats"  # v12
...
# ═══════════════════════════════════════════════════════════════════
# HEARTBEAT HELPERS (v12 — kills "alive but unresponsive" bug)
# ═══════════════════════════════════════════════════════════════════

def write_heartbeat(bot_id: str) -> None:
    """Each bot writes timestamp every HEARTBEAT_INTERVAL seconds."""
    try:
...
def normalize_support(value: str) -> str:
    ...
    # Keep https://t.me/+invite_hash style invite links as-is
...
def kill_process_tree(pid: int, sig: int = signal.SIGTERM) -> None:
    """Kill process and its entire group (v12 — kills lingering ffmpeg etc)."""
    try:
        pgid = os.getpgid(pid)
        os.killpg(pgid, sig)
    except Exception:
...
def spawn_clone_process(cfg_file: Path, bot_id: str) -> Optional[int]:
    """
    v12: FD-safe clone spawner.
    - close_fds=True → no inherited FDs (the bug that killed 5+ clones)
    - stdin=DEVNULL → no terminal binding
    - start_new_session → process group isolation
    - log file opened+closed correctly (no FD leak)
    """
    try:
        log_file_path = LOGS_DIR / f"{bot_id}.log"
        # Open log file, will be inherited briefly then closed in parent
        ...
        try:
            proc = subprocess.Popen(
                ...
                close_fds=True,
                start_new_session=True,  # os.setsid equivalent
                env={**os.environ, "ZUDO_CLONE_ID": bot_id},
            )
        finally:
        ...
        log.error("spawn_clone_process failed for %s: %s", bot_id, e)
        return None
...
class TelegramMusicBot:
    ...
    def __init__(self, config: BotConfig, config_path: Optional[Path] = None, is_master: bool = False):
        ...
        # v12: Per-clone isolated workdir
    ...
    # ═══════════════ BLOCKQUOTE PLAY-CARD (your screenshot style) ═══════════════

    def _play_card_text(self, track: Track) -> str:
        """
        v12: Each field wrapped in <blockquote> — Telegram renders this as
        the quoted-text style shown in your screenshot.
        """
        ...
        # requested_by may already contain HTML (escape_html applied at mention_user time)
        req = track.requested_by

        return (
            f"<b>🎶 {brand}  ᯤ‌  [ ɴᴏ ᴀᴅꜱ ] ♪✨</b>\n"
            f"{sep_thin()}\n\n"
    ...
    # ═══════════════════════════════════════════════════════════════
    # SEND PLAY CARD (with thumbnail + blockquote caption)
    # ═══════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════
    # PEER RESOLVER (kills PEER_ID_INVALID)
    # ═══════════════════════════════════════════════════════════════
        ...
        async with self.get_lock(message.chat.id):
            ...
            if state.current:
                ...
                return await self._safe_edit(
                    ...
                    f"<blockquote>🍎  <b>ᴛɪᴛʟᴇ</b>     :  {escape_html(track.title)}\n"
                    f"⚙️  <b>ᴅᴜʀᴀᴛɪᴏɴ</b>  :  {escape_html(track.pretty_duration)}\n"
                    f"💫  <b>ʀᴇǫᴜᴇꜱᴛᴇᴅ</b> :  {track.requested_by}</blockquote>"
                )
    ...
    async def _heartbeat_loop(self) -> None:
        """Write heartbeat every HEARTBEAT_INTERVAL seconds — proves bot is responsive."""
    ...
    async def _clone_watchdog(self) -> None:
        ...
        while not self._stopping:
            try:
                now = int(time.time())
                # Iterate over all configured clones
                for cfg_file in sorted(CLONES_DIR.glob("*.json")):
                    ...
                    if not alive:
                        ...
                        # No heartbeat yet — give it boot grace period
                        # check if pid file is older than CLONE_BOOT_GRACE
                        try:
                    ...
                    # Kill old process group if alive
                    if alive and pid:
                        try:
                    ...
                    if new_pid:
                        ...
                        log.error("Watchdog: clone %s respawn FAILED", bot_id)
        ...
        async def stats(_, m: Message):
            try:
                ...
                t = (
                    ...
                    f"🌐 ᴘᴇᴇʀ ᴄᴀᴄʜᴇ : {len(self._peer_resolved)}</blockquote>"
                )
                ...
                try:
                    ...
                    for f in files[:50]:
                        try:
                            ...
                            # v12: True health = alive AND responsive heartbeat
                            if healthy:
                                icon = "🟢"
                ...
                try:
                    ...
                    if step == "support":
                        ...
                        return await self._safe_send(m,
                            ...
                            f"  💡 ɪꜱ ᴄᴀ ɪꜱᴛᴇᴍᴀʟ ᴀʟᴀɢ ʜᴏᴛᴀ ʜᴀɪ — ᴜᴘᴅᴀᴛᴇꜱ ʙᴜᴛᴛᴏɴ ᴋᴇ ʟɪᴇ"
                        )
                    ...
                    if step == "session":
                        ...
                        try:
                            ...
                            await self._safe_send(m,
                                ...
                                f"  ✨ ᴀᴜᴛᴏ-ʀᴇꜱᴛᴀʀᴛ + ʜᴇᴀʀᴛʙᴇᴀᴛ ᴡᴀᴛᴄʜᴅᴏɢ ᴀᴄᴛɪᴠᴇ\n\n"
                                f"  🛑 ꜱᴛᴏᴘ ᴋᴀʀɴᴇ ᴋᴇ ʟɪᴇ:\n"
    ...
    async def start(self) -> None:
        ...
        # v12: Start heartbeat ASAP — proves we're responsive
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            pass
        # v12: Clear our own heartbeat so watchdog won't try to restart us
        try:
