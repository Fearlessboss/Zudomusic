...
"""
╔══════════════════════════════════════════════════════════════════╗
║   ♫  Z U D O  X  M U S I C   —   v 11  P R E M I U M  ♫        ║
║   Smart-Join Fix • Real-Ban Detection • Telegram Blockquote     ║
║   Separate Support + Updates Channels in /clone                 ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ NEW IN v11 (over v10) ═══════════════════
  ✅ ASSISTANT SMART-JOIN FIX:
       → "not joined ≠ banned" — proper status differentiation
       → Auto-join attempt with retry chain when assistant absent
       → False "banned" message ELIMINATED unless real BANNED status
       → Real ban detection ONLY via ChatMemberStatus.BANNED / KICKED
  ✅ TELEGRAM-NATIVE BLOCKQUOTE play card (your screenshot style)
       → Each field wrapped in <blockquote>...</blockquote>
       → Thumbnail caption uses proper Telegram blockquote rendering
  ✅ /clone FLOW — separate Support + Updates channels:
       Step 1: Bot Token
       Step 2: Support Chat link/username
       Step 3: Updates Channel link/username
       Step 4: Owner Username
       Step 5: Assistant Session
  ✅ All v10 features retained (peer-fix, friendly errors, etc.)
"""
...
def normalize_support(value: str) -> str:
    """Normalize support chat / channel — accepts @username, t.me/username, or full https link."""
...
def build_tme_url(value: str) -> str:
    """Build a clickable t.me URL from an @username or already-built https link."""
    value = (value or "").strip()
...
def load_config(path: Path) -> BotConfig:
    ...
    # backward-compat: old configs may lack updates_channel
...
# ═══════════════════════════════════════════════════════════════════
# FRIENDLY ERROR CLASSIFIER  (v11 — refined ban detection)
# ═══════════════════════════════════════════════════════════════════
...
def classify_error(
    ...
    *,
    ...
    """Translate raw exception into a friendly Hindi message."""
    ...
    # ── ASSISTANT BANNED (REAL ban only) ──
    if any(x in upper for x in (
...
class TelegramMusicBot:
    ...
    # ═══════════════ PREMIUM PLAY-CARD (Telegram blockquote style) ═══════════════

    def _play_card_text(self, track: Track) -> str:
        """Premium play card — every field a separate Telegram <blockquote>."""
    ...
    # ═══════════════════════════════════════════════════════════════
    # ULTIMATE PEER RESOLVER (kills PEER_ID_INVALID)
    # ═══════════════════════════════════════════════════════════════
    ...
    async def _get_assistant_member_status(self, chat_id: int) -> Tuple[str, Optional[Exception]]:
        """
        ✨ v11 — Returns a tuple (status, exc):
          status: "in" | "banned" | "left" | "unknown"
          exc: raised exception if applicable
        """
        try:
    ...
    async def _try_assistant_join(self, chat_id: int) -> Tuple[bool, Optional[Exception]]:
        """Attempt to add/join assistant via username → invite link fallback."""
        link = ""
        try:
            chat = await self.bot.get_chat(chat_id)
            uname = getattr(chat, "username", None)
    ...
    async def _ensure_assistant_in_chat(self, chat_id: int) -> Tuple[bool, Optional[Exception]]:
        """
        ✨ v11 — Smart assistant resolver:
          • If already member → resolve peer & return success.
          • If LEFT → attempt auto-join.
          • If BANNED → return real ban error.
          • If UNKNOWN → attempt join anyway.
        """
        now = time.time()

        # Recently joined? Just refresh peer.
        ...
        # Check member status
        status, status_exc = await self._get_assistant_member_status(chat_id)
        ...
        if status == "banned":
            # REAL ban — return UserBannedInChannel
            return False, UserBannedInChannel("User was banned in channel")

        # status == "left" or "unknown" → attempt auto-join
        joined, join_exc = await self._try_assistant_join(chat_id)
        if joined:
        ...
        # Join failed — return underlying error
        return False, (join_exc or status_exc or Exception("assistant join failed"))
            ...
            async def clone_flow_handler(_, m: Message):
                try:
                    ...
                    if step == "bot_token":
                        ...
                        return await self._safe_send(m,
                            f"  ✅ ᴛᴏᴋᴇɴ ꜱᴀᴠᴇᴅ\n\n"
                            f"  ꜱᴛᴇᴘ 2/5: 💬 <b>ꜱᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ</b> ʟɪɴᴋ / ᴜꜱᴇʀɴᴀᴍᴇ ʙʜᴇᴊᴏ\n\n"
                            f"  ᴇxᴀᴍᴘʟᴇ:\n"
                    ...
                    if step == "support":
                        ...
                        return await self._safe_send(m,
                            f"  ✅ ꜱᴜᴘᴘᴏʀᴛ ꜱᴀᴠᴇᴅ\n\n"
                            f"  ꜱᴛᴇᴘ 3/5: ✨ <b>ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ</b> ʟɪɴᴋ / ᴜꜱᴇʀɴᴀᴍᴇ ʙʜᴇᴊᴏ\n\n"
                            f"  ᴇxᴀᴍᴘʟᴇ:\n"
                    ...
                    if step == "updates":
                        ...
                        return await self._safe_send(m,
                            f"  ✅ ᴜᴘᴅᴀᴛᴇꜱ ꜱᴀᴠᴇᴅ\n\n"
                            f"  ꜱᴛᴇᴘ 4/5: 👤 <b>ᴏᴡɴᴇʀ ᴜꜱᴇʀɴᴀᴍᴇ</b> ʙʜᴇᴊᴏ\n\n"
                    ...
                    if step == "owner_username":
                        ...
                        return await self._safe_send(m,
                            f"  ✅ ᴏᴡɴᴇʀ ꜱᴀᴠᴇᴅ\n\n"
                            f"  ꜱᴛᴇᴘ 5/5: 🔑 <b>ᴀꜱꜱɪꜱᴛᴀɴᴛ ꜱᴇꜱꜱɪᴏɴ ꜱᴛʀɪɴɢ</b> ʙʜᴇᴊᴏ\n\n"
                    ...
                    if step == "session":
                        ...
                        try:
                            ...
                            await self._safe_send(m,
                                ...
                                f"  💬 ꜱᴜᴘᴘᴏʀᴛ: {escape_html(ccfg.support_chat)}\n"
                                f"  ✨ ᴜᴘᴅᴀᴛᴇꜱ: {escape_html(ccfg.updates_channel)}\n"
