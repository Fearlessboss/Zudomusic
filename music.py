...
═══════════════════ NEW IN v10 (over v9) ═══════════════════
  ...
  ✅ Premium <blockquote> play card (Telegram native quote style)
...
class TelegramMusicBot:
    ...
    def _play_card_text(self, track: Track) -> str:
        ...
        return (
            f"🎶 <b>{escape_html(self.display_name)}</b>  ᯤ‌  ✨\n"
            f"<i>{sep_thin()}</i>\n"
            f"<blockquote expandable>"
            f"🍎  <b>ᴛɪᴛʟᴇ</b>     :  <i>{escape_html(track.title)}</i>\n"
            f"⚙️  <b>ᴅᴜʀᴀᴛɪᴏɴ</b>  :  <code>{escape_html(track.pretty_duration)}</code>\n"
            f"💫  <b>ʀᴇǫᴜᴇꜱᴛᴇᴅ</b> :  {track.requested_by}\n"
            f"{mode_emoji}  <b>ᴍᴏᴅᴇ</b>      :  <i>{mode_label}</i>"
            f"</blockquote>"
        )
        ...
        async def ping(_, m: Message):
            try:
                ...
                t  = (
                    f"  💚 <b>{n}</b> ɪꜱ ᴏɴʟɪɴᴇ\n\n"
                    f"<blockquote>"
                    f"⚡  ʟᴀᴛᴇɴᴄʏ  :  <code>{ms:.2f} ᴍꜱ</code>\n"
                    f"⏳  ᴜᴘᴛɪᴍᴇ   :  <code>{escape_html(up)}</code>\n"
                    f"🎧  ᴀᴄᴛɪᴠᴇ   :  <code>{ac}</code> ᴄʜᴀᴛꜱ\n"
                    f"🤖  ʙᴏᴛ ɪᴅ   :  <code>{self.config.bot_id}</code>"
                )
                if self.is_master:
                    t += f"\n🔁  ᴄʟᴏɴᴇꜱ   :  <code>{cl}</code> ꜱᴀᴠᴇᴅ"
                t += "</blockquote>"
        ...
        async def stats(_, m: Message):
            try:
                ...
                t = (
                    f"  📊 <b>ʙᴏᴛ ꜱᴛᴀᴛꜱ</b>\n\n"
                    f"<blockquote>"
                    f"👥 ᴜꜱᴇʀꜱ  : <code>{len(self.known_users)}</code>\n"
                    f"💬 ᴄʜᴀᴛꜱ  : <code>{len(self.known_chats)}</code>\n"
                    f"🎧 ᴀᴄᴛɪᴠᴇ : <code>{sum(1 for s in self.states.values() if s.current)}</code>\n"
                    f"📥 ǫᴜᴇᴜᴇᴅ : <code>{sum(len(s.queue) for s in self.states.values())}</code>\n"
                    f"⏳ ᴜᴘᴛɪᴍᴇ : <code>{pretty_uptime(int(time.time() - self.start_time))}</code>\n"
                    f"🚦 ᴊᴏɪɴ ᴄᴀᴄʜᴇ : <code>{len(self._assistant_joined)}</code>\n"
                    f"🌐 ᴘᴇᴇʀ ᴄᴀᴄʜᴇ : <code>{len(self._peer_resolved)}</code>"
                    f"</blockquote>"
                )
