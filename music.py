...
║   ╚══════╝ ╚═════╝ ╚═════╝  ╚═════╝     ╚═╝  ╚═╝                ║
║                                                                  ║
║        ♫  Z U D O  X  M U S I C   —   v 6   P R O  ♫            ║
║      Zero Restart Loop • Stable Stream • Premium UI             ║
╚══════════════════════════════════════════════════════════════════╝

═══════════════════ FIXES IN v6 (ALL BUGS KILLED) ═══════════════════

  ✅ RESTART LOOP — KILLED (supervisor exits cleanly on /stop)
  ✅ Clone bots stay alive permanently (proper pid tracking)
  ✅ Private text handler conflict — FIXED (proper filter scoping)
  ✅ _schedule_save loop-safe (no more "no event loop" crash)
  ✅ PyTgCalls on_update — bulletproof (no exception propagation)
  ✅ Stream URL refresh on expiry (long songs play fully now)
  ✅ Signal handlers — graceful shutdown (no KeyboardInterrupt throw)
  ✅ YouTube bot detection bypass (android + ios + tv_embedded)
  ✅ PEER_ID_INVALID — auto-warm + retry
  ✅ Stream end detection — works on all py-tgcalls versions
  ✅ Premium stylish UI with gradient borders + animated emojis
"""
...
def _on_signal(signum, frame):
    print(f"\n[SIGNAL] Got signal {signum} — graceful shutdown initiated", flush=True)
    _SHUTDOWN_FLAG.set()
...
TRACK_CACHE_TTL   = int(os.getenv("TRACK_CACHE_TTL", "1800"))  # 30 min (stream URLs expire ~6h but safe side)
TRACK_CACHE_MAX   = 150
...
# ── Stylish UI separators ──
def sep() -> str:      return "╾━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╼"
def sep_thin() -> str: return "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
def sep_double() -> str: return "═══════════════════════════════"
...
def sync_extract_track(query: str, want_video: bool = False, use_cache: bool = True) -> Track:
    ...
    for idx in range(len(_YT_PLAYER_CLIENTS)):
        ...
        try:
            ...
            raise
    raise ValueError(
        f"ᴀʟʟ ᴄʟɪᴇɴᴛꜱ ꜰᴀɪʟ. ʟᴀꜱᴛ: {str(last_exc)[:200]}"
    )
...
class TelegramMusicBot:
    ...
    # ─────────────────────────────────────
    #  PERSISTENT STATE  (FIXED: loop-safe)
    # ─────────────────────────────────────
    ...
    def _schedule_save(self) -> None:
        """LOOP-SAFE save — uses the captured main loop."""
        try:
            ...
            if loop and loop.is_running():
                ...
                # Fallback: direct sync save
                self._save_state_sync()
        except Exception:
            # NEVER let save crash anything
            try:
                self._save_state_sync()
            except Exception:
                pass
    ...
    def _start_text(self, user_name: str = "") -> str:
        ...
        greet = f"  ✨ ʜᴇʏ <b>{escape_html(user_name)}</b>, ᴡᴇʟᴄᴏᴍᴇ! ✨" if user_name else "  ✨ ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴛʜᴇ ʙᴇᴀᴛ ʙᴏx ✨"
        return (
            f"┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓\n"
            f"┃   🎧  <b>{n.upper()}</b>  🎧   ┃\n"
            f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
            f"{greet}\n"
            f"{sep()}\n\n"
            f"❝ <i>ᴡʜᴇɴ ᴡᴏʀᴅs ꜰᴀɪʟ, ᴍᴜsɪᴄ sᴘᴇᴀᴋs.</i> ❞\n\n"
            f"{sep_thin()}\n\n"
            f"  ⚡  <b>ᴜʟᴛʀᴀ ꜰᴀꜱᴛ</b> ʏᴏᴜᴛᴜʙᴇ ᴘʟᴀʏʙᴀᴄᴋ\n"
            f"  🎶  <b>ꜱᴍᴀʀᴛ ǫᴜᴇᴜᴇ</b> + ʟᴏᴏᴘ + ꜱʜᴜꜰꜰʟᴇ\n"
            f"  🚀  <b>ɪɴꜱᴛᴀɴᴛ</b> ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ᴊᴏɪɴ\n"
            f"  💎  <b>ᴘʀᴇᴍɪᴜᴍ</b> ᴍᴜʟᴛɪ-ɢʀᴏᴜᴘ ꜱᴜᴘᴘᴏʀᴛ\n"
            f"  🛡  <b>ᴄʀᴀꜱʜ-ᴘʀᴏᴏꜰ</b> ᴀᴜᴛᴏ ʀᴇᴄᴏᴠᴇʀʏ\n\n"
            f"{sep_thin()}\n\n"
            f"  💫  <i>{tag}</i>\n\n"
            f"{sep()}\n\n"
            f"  🎵  <b>/play</b> <code>sᴏɴɢ ɴᴀᴍᴇ</code>  →  ɪɴsᴛᴀɴᴛ ᴍᴜsɪᴄ\n"
            f"  📚  ᴄʟɪᴄᴋ <b>ʜᴇʟᴘ</b> ʙᴜᴛᴛᴏɴ ɴɪᴄʜᴇ 👇"
        )
    ...
    def _about_text(self) -> str:
        ...
        return (
            ...
            f"  ⚡  ɪɴꜱᴛᴀɴᴛ ꜱᴏɴɢ ᴄᴀᴄʜᴇ\n"
            f"  📹  ᴀᴜᴅɪᴏ + ᴠɪᴅᴇᴏ ʙᴏᴛʜ ꜱᴜᴘᴘᴏʀᴛᴇᴅ\n\n"
            f"{sep()}\n\n"
            f"  🚀  <b>ɢʀᴏᴜᴘ ꜱᴇᴛᴜᴘ</b>\n"
            f"{sep_thin()}\n"
            f"  ❶  ʙᴏᴛ ᴀᴅᴅ ᴋᴀʀᴏ ɢʀᴏᴜᴘ ᴍᴇ\n"
            f"  ❷  ʙᴏᴛ ᴋᴏ ᴀᴅᴍɪɴ ʙᴀɴᴀᴏ\n"
            f"  ❸  ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ꜱᴛᴀʀᴛ ᴋᴀʀᴏ\n"
            f"  ❹  /play sᴏɴɢ ɴᴀᴍᴇ ʟɪᴋʜᴏ 🎶"
        )
    ...
    def _help_music_text(self) -> str:
        return (
            ...
            f"┃   🎵  ᴍᴜꜱɪᴄ ᴄᴏᴍᴍᴀɴᴅꜱ  🎵   ┃\n"
            f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
            f"  ▸  /play  <code>sᴏɴɢ / ᴜʀʟ</code>  →  🎵 ᴀᴜᴅɪᴏ\n"
            f"  ▸  /vplay <code>sᴏɴɢ / ᴜʀʟ</code>  →  📹 ᴠɪᴅᴇᴏ\n"
    ...
    def _help_admin_text(self) -> str:
        return (
            ...
            f"┃   🛠  ᴀᴅᴍɪɴ ᴄᴏɴᴛʀᴏʟꜱ  🛠   ┃\n"
            f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
            f"  🔁  /loop <code>[on/off]</code>  →  ʟᴏᴏᴘ ᴄᴜʀʀᴇɴᴛ\n"
            f"  🔀  /shuffle              →  ꜱʜᴜꜰꜰʟᴇ ǫᴜᴇᴜᴇ\n"
            f"  🧹  /clearqueue           →  ᴄʟᴇᴀʀ ǫᴜᴇᴜᴇ\n"
            f"  🔇  /mute                 →  ᴍᴜᴛᴇ ʙᴏᴛ\n"
            f"  🔊  /unmute               →  ᴜɴᴍᴜᴛᴇ ʙᴏᴛ\n\n"
            f"{sep_thin()}\n\n"
            f"  🏓  /ping   →  ʟᴀᴛᴇɴᴄʏ ᴄʜᴇᴄᴋ\n"
            f"  💚  /alive  →  ᴏɴʟɪɴᴇ ꜱᴛᴀᴛᴜꜱ\n\n"
            f"  ⚠️  <b>ᴀᴅᴍɪɴ-ᴏɴʟʏ ᴄᴏᴍᴍᴀɴᴅꜱ</b>\n"
            f"{sep()}"
        )
    ...
    def _help_extra_text(self) -> str:
        return (
            ...
            f"┃   🧩  ᴇxᴛʀᴀ ɪɴꜰᴏ  🧩   ┃\n"
            f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
            f"  ◈  ʙᴏᴛ ᴋᴏ ᴀᴅᴍɪɴ ʙᴀɴᴀᴏ ꜱᴍᴏᴏᴛʜ ᴍɢᴍᴛ ᴋᴇ ʟɪᴇ\n"
    ...
    def _shell_help_text(self) -> str:
        return (
            ...
            f"  ⚠️  <b>ꜱɪʀꜰ ᴏᴡɴᴇʀ ᴜꜱᴇ ᴋᴀʀ ꜱᴀᴋᴛᴀ</b>\n"
            f"{sep()}"
        )
    ...
    # ─────────────────────────────────────
    #  PLAY TRACK (with URL refresh)
    # ─────────────────────────────────────

    async def _refresh_track_if_stale(self, track: Track) -> Track:
        """Refresh stream URL if it might be expired (>25 min old)."""
        if time.time() - (track.fetched_at or 0) > 1500 and track.query:
            try:
                ...
                log.info("Refreshed stale stream URL for: %s", track.title[:50])
                return new_track
            except Exception as e:
                log.warning("Refresh failed, using old URL: %s", e)
        return track
    ...
    async def _play_track(self, chat_id: int, track: Track) -> Track:
        ...
        try:
            ...
            if "PEER_ID_INVALID" in err_text:
                log.warning("PEER_ID_INVALID — re-warming and retrying...")
                await self._warm_peer(chat_id)
                ...
                # Might be a stale URL — try refresh once
                if track.query and "403" in str(e) or "forbidden" in str(e).lower():
                    log.warning("Possibly stale URL, refreshing once...")
                    try:
                        invalidate_cached_track(track.query, track.is_video)
    ...
    async def _handle_play(self, message: Message, query: str, want_video: bool = False) -> None:
        ...
        try:
            await self._safe_edit(
                msg, self._np_text(self.get_state(message.chat.id)),
                reply_markup=self._np_kb()
            )
        except Exception:
            pass
            ...
            try:
                for pid_file in list(PIDS_DIR.glob("*.pid")):
                    ...
                    if not cfg_file.exists():
                        # Config deleted (dclone) — kill orphan and remove pid
                        try:
    ...
    async def _signal_watcher(self) -> None:
        """Watch shutdown flag and trigger graceful stop."""
        while not self._stopping:
            if _SHUTDOWN_FLAG.is_set():
                log.info("Shutdown flag set — stopping...")
                self._stopping = True
                # Stop pyrogram idle by stopping the bot
                try:
                    await self.bot.stop()
                except Exception:
                    pass
        ...
        if self.is_master:
            ...
            async def _dclone(_, m: Message):
                try:
                    ...
                    # Step 2: delete config so watchdog won't restart
                    cfg_removed = False
                    if cfg_f.exists():
            ...
            # ═══════════════════════════════════════════════════════
            #  CRITICAL FIX: Clone flow handler uses CUSTOM FILTER
            #  to ONLY trigger when user is in clone_flow dict.
            #  This prevents conflict with /play and other commands.
            # ═══════════════════════════════════════════════════════
                ...
                try:
                    ...
                    if step == "owner_username":
                        ...
                        return await self._safe_send(m,
                            ...
                            f"  (/default = ꜱᴀᴍᴇ ᴀꜱꜱɪꜱᴛᴀɴᴛ ʀᴀᴋʜᴏ)"
                        )
    ...
    async def start(self) -> None:
        # Capture main loop for thread-safe save
        ...
        # ═══════════════════════════════════════════════════════════
        #  IDLE LOOP — KEY FIX: idle() exception does NOT cause
        #  infinite PyTgCalls restart. We just exit cleanly.
        # ═══════════════════════════════════════════════════════════
        try:
            await idle()
...
# ═══════════════════════════════════════════════════════════════════
#  ENTRY POINT — RESTART LOOP KILLED
# ═══════════════════════════════════════════════════════════════════

async def run_once() -> bool:
    """Returns True if bot stopped due to shutdown signal (don't restart),
       False if it crashed (may restart)."""
    ...
    try:
        ...
        await app.stop()
    # Return True if shutdown was requested (no restart needed)
    return _SHUTDOWN_FLAG.is_set()


async def supervisor() -> None:
    """KEY FIX: Only restarts on actual CRASH, not on clean exit.
       Shutdown signal exits immediately."""
    ...
    while True:
        ...
        # Check clone config exists
        if _is_clone:
            cfg_path = Path(sys.argv[2]).resolve()
            if not cfg_path.exists():
                log.info("Clone config deleted (/dclone'd). Supervisor exiting.")
                # Clean up pid file
                try:
        ...
        try:
            ...
            if shutdown_requested:
                ...
                return
            # Bot exited without crash and without shutdown signal
            # This means it stopped cleanly somehow — exit too
            log.info("Bot exited cleanly without crash — supervisor exiting")
            return
        ...
        # Only restart if we actually crashed AND not shutting down
        if _SHUTDOWN_FLAG.is_set():
...
def main():
    # Register signal handlers — set flag, don't throw
